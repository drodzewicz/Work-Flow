...
export class SeedService {
    ...
    async generateBoardTags(
        ...
        return await Promise.all(
            tags.map(({ name, key }) => this.tagService.createTag(boardId, { name, key }))
        );
    }
    ...
    async generateTasks(
        ...
        for (const i in tasks) {
            ...
            const persistedTask = await this.taskService.createTask(
                { title, description },
                boardId,
                author._id
            );
    ...
    async generateBoards(
        ...
        return await Promise.all(
            boards.map(async (board) => {
                ...
                const persistedBoard = await this.boardService.createBoard(
                    { name, description },
                    boardAuthor._id
                );
                ...
                const persistedColumns = await Promise.all(
                    columns.map((columnName) =>
                        this.boardService.createColumn(persistedBoard._id, columnName)
                    )
                );
                // add other users to the board
                await Promise.all(
                    boardMembers.map((user) =>
                        this.memberService.addUserToBoard(persistedBoard._id, user._id)
                    )
                );
