...
class ColumnElement extends BasePage {
    ...
    async delete(columnName: string) {
        await this.getByName(columnName)
            .getByTestId("column-option-btn")
            .click();
    ...
    async move(sourceColumnName: string, destinationColumnName: string) {
        const sourceColumn =
            this.getByName(sourceColumnName).getByRole("banner");
        const destinationColumn = this.getByName(
