...
class TaskCardElement extends BasePage {
  ...
  getAssignees(taskName: string) {
    return this.getByName(taskName).getByRole("img", { name: "task-assignee" });
  }
  ...
  async move(taskName: string, destinationColumnName: string, index: number) {
    ...
    if (taskCount === 0) {
      ...
      if (box) {
        coordinates = { x: box.x + box.width / 2 + 100, y: box.y + box.height / 2 };
      }
    } else {
      if (index < taskCount) {
        box = await tasksFromColumn.nth(index).boundingBox();
        if (box) {
          coordinates = { x: box.x + box.width / 2 + 100, y: box.y + box.height / 2 - 10 };
        }
      } else if (index >= taskCount) {
        box = await tasksFromColumn.last().boundingBox();
        if (box) {
          coordinates = { x: box.x + box.width / 2 + 100, y: box.y + box.height / 2 + 100 };
        }
      } else {
